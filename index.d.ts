...
declare class Tone {
	 ...
	 */
	static extend<T extends Tone.Ctor, P extends Tone.Ctor = Tone.Ctor<any[], Tone>>(
		child: T,
		parent?: P
	 ...
	 *  @return  {Tone}
	 */
	static getContext(arg: (context: Tone.Context) => void): Tone;
...
declare namespace Tone {
	...
	export type ProcessingNode = Tone | AudioNode | AudioParam | AudioNode;
	...
	interface TransportTimeConstructor
		extends PrimitiveTypeConstructor<Encoding.TransportTime, TransportTime> {}
	...
	interface Transport extends Emitter {
		...
		scheduleRepeat(
			...
			startTime?: Encoding.TransportTime,
	...
	class Param<E = Encoding.Default, T = number> extends AudioNode {
		...
		setValueCurveAtTime(
			...
			startTime: Encoding.Time,
			duration: Encoding.Time,
			scaling?: Encoding.NormalRange
		): this;
	...
	type EnvelopeCurve = 'linear' | 'exponential' | 'sine' | 'cosine' | 'bounce' | 'ripple' | 'step' | number[];
	...
	class LowpassCombFilter extends AudioNode {
		constructor(
			delayTime?: Encoding.Time | object,
	...
	class Part extends Event {
		constructor(
			callback: Callback<[Encoding.Time, TimeEventObject]>,
			events: ReadonlyArray<TimeEventPair | TimeEventObject>
		);
	...
	class BufferSource extends AudioNode {
		...
		start(
			startTime?: Encoding.Time,
			offset?: Encoding.Time,
			duration?: Encoding.Time,
			gain?: Encoding.Gain,
			fadeInTime?: Encoding.Time
		): this;
	...
	class FatOscillator extends Source {
		constructor(
			frequency: Encoding.Frequency,
			type: BasicOscillatorType,
			modulationType: BasicOscillatorType
		);
